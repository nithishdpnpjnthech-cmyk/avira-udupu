...
const ProductDetailPage = () => {
  ...
  const productId = routeId || searchParams?.get('id') || '1';
  ...
  const resolveImageUrl = (candidate) => {
    ...
    // Default bare filename to uploads route
    ...
    // Normalize uploads path away from /api prefix
  ...
  useEffect(() => {
    const loadProduct = async () => {
      try {
        ...
        // Initialize with first variant's images if available, otherwise use product images
        const firstVariant = normalizedProduct.variants?.[0];
        if (firstVariant) {
          const variantImages = [
            firstVariant.mainImage,
            firstVariant.subImage1,
            firstVariant.subImage2,
            firstVariant.subImage3
          ].filter(Boolean);
          setCurrentImages(variantImages.length > 0 ? variantImages : normalizedProduct.images || []);
        } else {
          setCurrentImages(normalizedProduct.images || []);
        }
  ...
  const breadcrumbItems = [
    ...
    { label: product?.category || 'Category', path: `/product-collection-grid?category=${product?.category}`},
  ...
  const handleVariantChange = (variant) => {
    ...
    const variantImages = [];
    if (variant.mainImage) variantImages.push(variant.mainImage);
    if (variant.subImage1) variantImages.push(variant.subImage1);
    if (variant.subImage2) variantImages.push(variant.subImage2);
    if (variant.subImage3) variantImages.push(variant.subImage3);
    
    // If variant has images, use them; otherwise keep product default images
