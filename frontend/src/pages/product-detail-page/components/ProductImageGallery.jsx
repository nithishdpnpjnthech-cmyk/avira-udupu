...
const ProductImageGallery = ({ images, productName }) => {
  ...
  return (
    <div className="space-y-4">
      {/* Main Image */}
      <div className="relative flex items-center justify-center bg-muted rounded-lg overflow-visible w-full h-[500px] py-4 px-4">
        <Image
          src={images?.[selectedImageIndex]}
          alt={`${productName} - Image ${selectedImageIndex + 1}`}
          className={`max-w-full max-h-full w-auto h-auto object-contain transition-transform duration-300 cursor-zoom-in ${
        ...
        <div className="flex gap-3 overflow-x-auto pb-2">
          {images?.map((image, index) => (
            <button
              ...
              className={`flex-shrink-0 w-20 h-28 rounded-lg overflow-hidden border-2 transition-colors duration-200 flex items-center justify-center bg-muted ${
                index === selectedImageIndex
                  ? 'border-royal-blue' :'border-border hover:border-royal-blue/50'
              }`}
            >
              <Image
                src={image}
